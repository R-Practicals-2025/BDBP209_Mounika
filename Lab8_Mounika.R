...
# Task 5: Find largest and second-largest words
...
k='123'
print()
phrase <- "abcde fghijkl mnopqr stuv wxyz.chtduyfyjg,vjhgjg"

for (i in order(unlist(strsplit(phrase, "")))) {
  print(substring(phrase, i))
}

# Splitting by spaces (words)
print(strsplit(phrase, split = " "))

# Splitting by characters
print(strsplit(phrase, split = ""))

# Counting the number of characters in each word and tabulating
print(table(unlist(lapply(strsplit(phrase, split = " "), nchar))))

print(strsplit(phrase,split = NULL))

# Reverse the entire string
reversed_phrase <- paste(rev(strsplit(phrase, NULL)[[1]]), collapse = "")
...
# Split phrase into words and store in a vector
vec <- unlist(strsplit(phrase, split = " "))
...
#using table fun
print(table(strsplit(phrase,NULL)))

column(phrase)
print(toupper(phrase))

first <- c(1,2,3,4,5,3,6,3,3,3,6,7)
sec <- c(1,2,3,4)
match(first,sec)
 
SUB <- c("A","B","C","D","M","V","A","A")
SUB1 <- c("A","A","A","n","z")
match(first,sec)

y =c(1,2,-1,2,4,6,8,9,-10,23,11,1)
z <- ifelse(y<0,-1,1)
print(z)


y =c(1,2.1,-1,2,4.9,6,8,9,-1.0,23,11,1)
print(median(y))
...
try <- c("hello","namaste")
try1 <- match(SUB,SUB1)
try_suggest <- try[ifelse(is.na(try1),2,1)]
print(try_suggest)


text <- c("naaana","nevjf","213456","-hevf")
print(gsub("n","M",text))
print(sub("n","M",text))
print(gsub("^.","O",text))

s <- "filemame.doc"
print(strsplit(s,split = " ."))
print(strsplit(s,split = "\\."))

#grep
print(grep("na{1}",text,value=T))
print(grep('[[:alnum:]]{4,}',text,value=T))

string2 <- "ASDGGhhkhkhk"
print(strtrim (string2,4))

s="aacysjbfhrghdjhaacy"
print(grep(a{2}c{1}y,s))
